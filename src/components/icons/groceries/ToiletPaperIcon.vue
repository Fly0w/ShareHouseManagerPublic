<template>
  <svg
    version="1.1"
    id="Layer_1"
    xmlns="http://www.w3.org/2000/svg"
    xmlns:xlink="http://www.w3.org/1999/xlink"
    viewBox="0 0 512 512"
    xml:space="preserve"
  >
    <polyline
      style="fill: #c7c5cc"
      points="86.724,334.013 86.724,478.362 126.185,441.008 177.293,478.362 236.731,441.008 
	298.116,478.362 358.19,441.008 412.885,478.362 462.442,441.008 512,478.362 512,181.454 "
    />
    <path
      style="fill: #f8f8f8"
      d="M423.251,333.36H87.08C38.987,333.36,0,266.266,0,183.501S38.987,33.642,87.08,33.642h336.171"
    />
    <ellipse style="fill: #e1e1e4" cx="424.913" cy="183.498" rx="87.085" ry="149.86" />
    <path
      style="fill: #c7c5cc"
      d="M426.27,112.955c-25.238,0-45.005,30.988-45.005,70.546s19.769,70.546,45.005,70.546
	s45.007-30.988,45.007-70.546S451.508,112.955,426.27,112.955z M426.27,234.883c-12.223,0-25.843-21.103-25.843-51.384
	s13.62-51.384,25.843-51.384c12.224,0,25.845,21.103,25.845,51.384S438.494,234.883,426.27,234.883z"
    />
  </svg>
</template>

<script>
export default {
  name: 'ToiletPaperIcon'
}
</script>
