<template>
  <svg
    version="1.1"
    id="_x32_"
    xmlns="http://www.w3.org/2000/svg"
    xmlns:xlink="http://www.w3.org/1999/xlink"
    viewBox="0 0 512 512"
    xml:space="preserve"
  >
    <g>
      <path
        class="st0"
        d="M168.299,86.264c23.815,0,43.127-19.304,43.127-43.128C211.426,19.312,192.114,0,168.299,0
			c-23.832,0-43.144,19.312-43.144,43.136C125.155,66.959,144.467,86.264,168.299,86.264z"
      />
      <path
        class="st0"
        d="M389.331,132.104v-0.851c0-12.779-10.352-23.131-23.13-23.131H171.904c-46.82,0-88.396,15.612-98.772,67.516
			L41.447,307.916c-2.228,9.962,4.042,19.829,14.005,22.057c9.946,2.22,19.813-4.05,22.033-14.004l33.332-88.132l8.625,262.281
			c0,12.087,9.795,21.882,21.874,21.882c12.095,0,21.891-9.796,21.891-21.882l8.697-156.413l8.681,156.413
			c0,12.087,9.803,21.882,21.882,21.882c12.087,0,21.891-9.796,21.891-21.882l12.309-316.748l132.708-18.357
			C380.809,153.429,389.331,143.65,389.331,132.104z"
      />

      <rect
        x="336.541"
        y="225.644"
        transform="matrix(-0.7648 0.6443 -0.6443 -0.7648 820.1971 192.6535)"
        class="st0"
        width="76.783"
        height="40.794"
      />
      <path
        class="st0"
        d="M336.543,448.804c0,9.286,7.52,16.806,16.797,16.806c9.27,0,16.798-7.52,16.798-16.806
			c0-9.278-7.528-16.789-16.798-16.789C344.063,432.015,336.543,439.526,336.543,448.804z"
      />
      <rect x="377.332" y="381.615" class="st0" width="35.998" height="35.998" />
      <polygon
        class="st0"
        points="442.046,297.636 422.424,483.203 322.603,483.203 302.981,297.636 274.033,297.636 296.695,512 
			448.333,512 471.003,297.636 	"
      />
    </g>
  </svg>
</template>

<script>
export default {
  name: 'ThrowAwayIcon'
}
</script>
